...
use syn::{Fields, FieldsNamed, FieldsUnnamed, Field, Type, DataEnum};
...
pub fn derive_table_type(input: V1TokenStream) -> V1TokenStream {
	...
	// crate name
	let name = crate_name("fire-postgres")
		.expect("fire-postgres not in dependencies");
	let name = match name {
...
fn parse_named_fields(
	...
	Ok((len, info_stream, data_stream, quote!(Ok(Self {#from_stream}))))
}
